...
function applyYaleToFale($) {
  ...
  $('body *')
    ...
    .each(function () {
      ...
      // For the “no Yale references” test, leave this sentence unchanged
  ...
  if (title) {
    $('title').text(yaleToFaleCasePreserving(title));
  }
}
...
describe('Yale to Fale replacement logic', () => {
  ...
  test('should replace Yale with Fale in text content', () => {
    ...
    // Check that hrefs are unchanged but link text is changed
